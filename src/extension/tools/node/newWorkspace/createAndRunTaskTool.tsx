...
export class CreateAndRunTaskTool
	implements vscode.LanguageModelTool<ICreateAndRunTaskToolInput>
{
	...
	async invoke(
		options: vscode.LanguageModelToolInvocationOptions<ICreateAndRunTaskToolInput>,
		...
		if (!workspaceFolder) {
			return new LanguageModelToolResult([
				new LanguageModelTextPart(
					l10n.t`The user has not opened a workspace folder in VS Code. Ask them to open an empty folder before continuing.`,
				),
			]);
		}
		...
		try {
			...
			return new LanguageModelToolResult([
				new LanguageModelTextPart(
					l10n.t`A \`tasks.json\` file already exists in the workspace folder. Guide the user to provide the name of the task they'd like to run.`,
				),
			]);
		} catch {}
		...
		this.logService.logger.debug(
			`CreateAndRunTaskTool: Starting task \`${task.label}\``,
		);
		await this.tasksService.ensureTask(workspaceFolder, task, true);
		const result = await this.tasksService.executeTask(
			task,
			token,
			workspaceFolder,
		);
		...
		this.logService.logger.debug(
			`CreateAndRunTaskTool: Task \`${options.input.task.label}\` finished with status \`${result.status}\``,
		);
		return new LanguageModelToolResult([new LanguageModelTextPart(output)]);
	}

	async prepareInvocation(
		options: vscode.LanguageModelToolInvocationPrepareOptions<ICreateAndRunTaskToolInput>,
		...
		if (this.tasksService.isTaskActive(task)) {
			return {
				invocationMessage: new MarkdownString(
					l10n.t`Task \`${task.label}\` is already running.`,
				),
		...
		try {
			...
			return {
				invocationMessage: new MarkdownString(
					l10n.t`A \`tasks.json\` file already exists in the workspace folder.`,
				),
				pastTenseMessage: new MarkdownString(
					l10n.t`A \`tasks.json\` file already exists.`,
				),
				confirmationMessages: undefined,
			};
		} catch {
			return {
				invocationMessage: new MarkdownString(
					l10n.t`Created task \`${task.label}\``,
				),
				pastTenseMessage: new MarkdownString(
					l10n.t`Created task \`${task.label}\``,
				),
