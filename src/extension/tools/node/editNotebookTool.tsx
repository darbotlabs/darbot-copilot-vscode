...
export class EditNotebookTool implements ICopilotTool<IEditNotebookToolParams> {
	...
	async invoke(
		...
		try {
			...
			const cellCode = Array.isArray(newCode)
				? newCode.join(EOL)
				: newCode;
			...
			if (editType === 'insert') {
				...
				previousCellIdUsedForInsertion = cellId =
					cellId || previousCellIdUsedForInsertion;
				...
				if (cellId === 'top') {
					...
					const cell = cellId
						? provider.getCell(notebook, cellId)
						: undefined;
					if (cellId && !cell) {
						throw new ErrorWithTelemetrySafeReason(
							`Invalid cell id: ${cellId}, notebook may have been modified, try reading the file again`,
				...
				previousCellIdUsedForInsertion =
					cellId ?? previousCellIdUsedForInsertion;
				const languageId =
					language || getDefaultLanguage(notebook) || 'python'; // Default to Python if no language is provided
				const cellKind =
					language === 'markdown'
				...
				if (editType === 'delete') {
					...
					if (
						existingCell.document.getText().length &&
						cellCode.includes(EXISTING_CODE_MARKER)
					) {
						...
						const requestInput = createCodeMapperRequest(
							existingCell,
							notebookUri,
							cellCode,
							explanation ?? '',
						);
						const cellEditOperation = codeMapper.mapCode(
							requestInput,
							{
								notebookEdit(target, edits) {
									stream.notebookEdit(target, edits);
								},
	...
	prepareInvocation(
		...
		return {
			invocationMessage:
				options.input.explanation || l10n.t('Editing notebook'),
			presentation:
				options.input.explanation && typeof options.input.explanation
					? undefined
					: 'hidden',
		};
	}
	...
	private validateInput(
		...
		const id = (
			(typeof (cellId as any) === 'number' ? `${cellId}` : cellId) || ''
		).trim();
		const cell =
			id && id !== 'top' && id !== 'bottom'
				? provider.getCell(notebook, id)
				: undefined;
		if (id && id !== 'top' && id !== 'bottom' && !cell) {
			throw new ErrorWithTelemetrySafeReason(
				`None of the edits were applied as cell id: ${id} is invalid. Notebook may have been modified, try reading the file again`,
				'invalidCellId',
			);
		}
		switch (editType) {
			...
			case 'delete':
				if (id === undefined) {
					throw new ErrorWithTelemetrySafeReason(
						'None of the edits were applied as cellId is required for delete operation',
						'missingCellId',
					);
				}
				break;
			case 'edit':
				if (!id) {
					throw new ErrorWithTelemetrySafeReason(
						'None of the edits were applied as cellId is required for edit operation',
						'missingCellId',
					);
				}
				if (newCode === undefined) {
	...
	private fixInput(
		...
		if (input.editType === 'insert' && input.newCode) {
			input.newCode = provider.stripCellMarkers(
				Array.isArray(input.newCode)
					? input.newCode.join(EOL)
					: input.newCode,
			);
		}
		if (input.newCode && Array.isArray(input.newCode)) {
			input.newCode = Array.isArray(input.newCode)
				? input.newCode.join(EOL)
				: input.newCode;
		}
...
function summarizeOriginalEdits(
	notebook: vscode.NotebookDocument,
	inputEdit: IEditNotebookToolParams,
	edits: ChangedCell[],
): string {
...
function createCodeMapperRequest(
	existingCell: vscode.NotebookCell,
	...
	const documentSnapshot = TextDocumentSnapshot.fromJSON(
		existingCell.document,
		{
...
async function sendEditNotebookCellOperationsTelemetry(
	...
	endpointProvider: IEndpointProvider | undefined,
	options: vscode.LanguageModelToolInvocationOptions<IEditNotebookToolParams>,
	counters: { insert: number; edit: number; delete: number },
) {
	const model =
		options.model &&
		endpointProvider &&
