...
class VSCodeCmdTool
	implements vscode.LanguageModelTool<IVSCodeCmdToolToolInput>
{
	...
	async invoke(
		...
		if (!commandItem) {
			return new LanguageModelToolResult([
				new LanguageModelTextPart(
					`Failed to find ${options.input.name} command.`,
				),
			]);
		}
		...
		try {
			...
			return new LanguageModelToolResult([
				new LanguageModelTextPart(
					`Finished running ${options.input.name} command`,
				),
			]);
		} catch (error) {
			return new LanguageModelToolResult([
	...
	async prepareInvocation(
		...
		const markdownString = new MarkdownString(
			l10n.t(
				`Copilot will execute the [{0}](command:workbench.action.quickOpen?{1}) command.`,
				options.input.name,
				query,
			),
		);
