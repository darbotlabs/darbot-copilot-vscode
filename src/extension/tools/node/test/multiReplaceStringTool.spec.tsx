	...
	test('multi-sr bug', async () => {
		const input: IMultiReplaceStringToolParams = {
			...
			replacements: [
				{
					explanation:
						'Update imports to include IModifiedFileEntryInternal',
					filePath: '/workspace/multi-sr-bug.ts',
					newString:
						"import { ChatEditingSessionState, ChatEditKind, getMultiDiffSourceUri, IChatEditingSession, IModifiedEntryTelemetryInfo, IModifiedFileEntry, IModifiedFileEntryInternal, IPendingFileOperation, ISnapshotEntry, IStreamingEdits, ModifiedFileEntryState } from '../../common/chatEditingService.js';",
					oldString:
						"import { ChatEditingSessionState, ChatEditKind, getMultiDiffSourceUri, IChatEditingSession, IModifiedEntryTelemetryInfo, IModifiedFileEntry, IPendingFileOperation, ISnapshotEntry, IStreamingEdits, ModifiedFileEntryState } from '../../common/chatEditingService.js';",
				},
				{
...
import { IFile } from '../../../../../base/node/zip.js';`,
				...
				{
					explanation:
						'Update _entriesObs type to use IModifiedFileEntryInternal',
					filePath: '/workspace/multi-sr-bug.ts',
	...
	public get entries(): IObservable<readonly IModifiedFileEntry[]> {
		...
		const r = await invoke(input);
		expect(await applyEditsInMap(r.edits)).toMatchFileSnapshot(
