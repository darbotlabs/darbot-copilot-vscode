...
import {
	...
	TaskResult,
	TaskStatus,
} from '../../../platform/tasks/common/tasksService';
...
class RunTaskTool implements vscode.LanguageModelTool<IRunTaskToolInput> {
	...
	async invoke(
		options: vscode.LanguageModelToolInvocationOptions<IRunTaskToolInput>,
		token: CancellationToken,
	): Promise<vscode.LanguageModelToolResult> {
		const task = this.getTaskDefinition(options.input)?.task;
		if (task && this.tasksService.isTaskActive(task)) {
			return new LanguageModelToolResult([
				new LanguageModelTextPart(l10n.t`The task is already running.`),
			]);
		}

		const workspaceFolderRaw =
			this.promptPathRepresentationService.resolveFilePath(
				options.input.workspaceFolder,
			);
		const workspaceFolder =
			(workspaceFolderRaw &&
				this.workspaceService.getWorkspaceFolder(workspaceFolderRaw)) ||
			this.workspaceService.getWorkspaceFolders()[0];

		const totalStartTime = Date.now();
		const taskStartTime = totalStartTime;
		const result: TaskResult = task
			? await this.tasksService.executeTask(task, token, workspaceFolder)
			: { status: TaskStatus.Error, error: new Error('Task not found') };
		...
		if (task) {
			...
			for (const interval of checkIntervals) {
				...
				const buffer = this.terminalService.getBufferForTerminal(
					terminal,
					16000,
				);
				...
				if (idleCount >= 2 || inactive) {
					...
					pollDurationMs =
						pollEndTime - (pollStartTime ?? pollEndTime);
					idleOrInactive = inactive ? 'inactive' : 'idle';
					const evalStartTime = Date.now();
					const evalResult = await this._evaluateOutputForErrors(
						buffer,
						token,
					);
					...
					return new LanguageModelToolResult([
						new LanguageModelTextPart(l10n.t`${evalResult}`),
					]);
				}
			}
		}
		...
		switch (result.status) {
			case TaskStatus.Started:
				output =
					'Task started and will continue to run in the background.';
				break;
	...
	private async _evaluateOutputForErrors(
		output: string,
		token: CancellationToken,
	): Promise<string> {
		const endpoint =
			await this._endpointProvider.getChatEndpoint('gpt-4o-mini');
					...
					content: [
						{
							...
							text: `Review this output to determine if the task exited or if there are errors ${output}. If it has exited, explain why.`,
						},
					],
				},
			],
	...
	async prepareInvocation(
		options: vscode.LanguageModelToolInvocationPrepareOptions<IRunTaskToolInput>,
		token: vscode.CancellationToken,
	): Promise<vscode.PreparedToolInvocation> {
		const { task, workspaceFolder, taskLabel } =
			this.getTaskDefinition(options.input) || {};

		const position =
			workspaceFolder &&
			task &&
			(await this.tasksService.getTaskConfigPosition(
				workspaceFolder,
				task,
			));
		const link = (s: string) =>
			position
				? `[${s}](${position.uri.toString()}#${position.range.startLineNumber}-${position.range.endLineNumber})`
				: s;
		...
		if (task && this.tasksService.isTaskActive(task)) {
			return {
				invocationMessage: trustedMark(
					l10n.t`${link(taskLabel ?? options.input.id)} is already running.`,
				),
				pastTenseMessage: trustedMark(
					l10n.t`${link(taskLabel ?? options.input.id)} was already running.`,
				),
		...
		return {
			invocationMessage: trustedMark(
				l10n.t`Running ${taskLabel ?? link(options.input.id)}`,
			),
			pastTenseMessage: trustedMark(
				task?.isBackground
					? l10n.t`Started ${link(taskLabel ?? options.input.id)}`
					: l10n.t`Ran ${link(taskLabel ?? options.input.id)}`,
			),
			confirmationMessages:
				task && task.group !== 'build'
					? {
							title: l10n.t`Allow task run?`,
							message: trustedMark(
								l10n.t`Allow Copilot to run the \`${task.type}\` task ${link(`\`${this.getTaskRepresentation(task)}\``)}?`,
							),
						}
					: undefined,
		};
	...
	private getTaskDefinition(input: IRunTaskToolInput) {
		...
		const task = this.tasksService
			...
			.find(
				(t, i) =>
					t.type === taskType && (t.label || String(i)) === taskLabel,
			);
		if (!task) {
			return undefined;
		}
