...
import {
	TestFailureList,
	TestFailureListElementProps,
} from './testFailureTool';
...
export class RunTestsTool implements ICopilotTool<IRunTestToolsInput> {
	...
	async invoke(
		{
			input,
			tokenizationOptions,
		}: LanguageModelToolInvocationOptions<IRunTestToolsInput>,
		...
		await new Promise<void>((resolve) => {
			store.add(
				this.testProvider.onDidChangeResults(() => {
		...
		const json = await renderPromptElementJSON(
			...
			TestResult,
			{ failures: [...this.testProvider.getAllFailures()] },
			tokenizationOptions,
		);
	...
	prepareInvocation?(
		options: LanguageModelToolInvocationPrepareOptions<IRunTestToolsInput>,
		token: CancellationToken,
	): PreparedToolInvocation {
		const title = l10n.t`Allow test run?`;
		const inFiles = options.input.files?.map(
			(f) => '`' + basename(f) + '`',
		);
		...
		return {
			...
			confirmationMessages: {
				...
				message: inFiles?.length
					? new MarkdownString(
							l10n.t`The model wants to run tests in ${inFiles.join(', ')}.`,
						)
	...
	render() {
		return (
			<>
				<Tag
					name="testResult"
					attrs={{ passed: this.props.failures.length === 0 }}
				>
